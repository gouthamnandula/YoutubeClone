...
class Video(models.Model):
    ...
    @property  #video.display_thumbnail_url -> Decide which thumbnail to show
    ...
    @property  #Create thumbnail from video automatically
    def generated_thumbnail_url(self):
        if not self.video_url:
            return ""
        return get_thumbnail_url(self.video_url, self.user.username)

    @property #Convert raw video URL into a streamable format
    def streaming_url(self):
        if not self.video_url:
            return ""
        return get_streaming_url(self.video_url)

    @property #Return a compressed / optimized version of video
