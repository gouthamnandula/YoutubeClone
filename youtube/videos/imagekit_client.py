...
def get_optimized_video_url(base_url: str) -> str: #getting url for loading the video
    if "?" in base_url:
        return f"{base_url}&tr=q-50,f-auto" #optimizatiion parameter for fast loading of video, refer docs of imagekit
    return f"{base_url}?tr=q-50,f-auto"


def get_streaming_url(base_url: str) -> str:      #ABS in imagekit -> Application bitrate streaming setting
    return f"{base_url}/ik-master.m3u8?tr=sr-240_360_480_720_1080"


def get_thumbnail_url(  #getting url for thumbnail
...
def upload_video(file_data: bytes, file_name: str, folder: str = "videos") -> dict: #if not dict it will become tuple output and become messier
